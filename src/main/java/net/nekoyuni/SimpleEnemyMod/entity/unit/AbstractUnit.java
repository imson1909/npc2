...
public abstract class AbstractUnit extends Monster implements IRoleHolder, IAnimatedEntity {
   ...
   public static final EntityDataAccessor<BlockPos> HOME_POS = SynchedEntityData.defineId(AbstractUnit.class, EntityDataSerializers.BLOCK_POS);
   public static final EntityDataAccessor<String> FACTION_TYPE = SynchedEntityData.defineId(AbstractUnit.class, EntityDataSerializers.STRING);
   public static final EntityDataAccessor<Long> LAST_SEEN_TARGET_TIME = SynchedEntityData.defineId(AbstractUnit.class, EntityDataSerializers.LONG);
   ...
   protected static final long TARGET_LOST_RETURN_DELAY = 600L;
   protected static final double HOME_PATROL_RADIUS = 15.0;
   protected static final double HOME_MAX_DISTANCE_SQR = 225.0;

   // Turn speed - degrees per tick
   ...
   protected void defineSynchedData() {
      ...
      this.entityData.define(FACTION_TYPE, FactionType.TOWER.name());
      this.entityData.define(LAST_SEEN_TARGET_TIME, 0L);
   }
   ...
   public SpawnGroupData finalizeSpawn(
      ...
      if (getHomePoint() == null) {
         setHomePoint(this.blockPosition());
      }
   ...
   public void addAdditionalSaveData(CompoundTag tag) {
      ...
      tag.putLong("LastSeenTarget", (Long)this.entityData.get(LAST_SEEN_TARGET_TIME));
   }
   ...
   public void readAdditionalSaveData(CompoundTag tag) {
      ...
      if (tag.contains("HomePos")) {
         setHomePoint(BlockPos.of(tag.getLong("HomePos")));
      }
      if (tag.contains("Faction")) {
         try {
            setFaction(FactionType.valueOf(tag.getString("Faction")));
         } catch (IllegalArgumentException e) {
            setFaction(FactionType.TOWER);
         }
      }
      if (tag.contains("LastSeenTarget")) {
         this.entityData.set(LAST_SEEN_TARGET_TIME, tag.getLong("LastSeenTarget"));
      }
      this.setupRoleGoals();
   }
   ...
   public void tick() {
      ...
      if (!this.level().isClientSide()) {
         ...
         if (target != null && target.isAlive()) {
            ...
            if (this.hasLineOfSight(target)) {
               this.entityData.set(LAST_SEEN_TARGET_TIME, this.level().getGameTime());
            }
         }

         if (this.getTarget() == null && this.isTargetLostTimeoutExpired()) {
            BlockPos home = this.getHomePoint();
            if (home != null && this.distanceToSqr(Vec3.atCenterOf(home)) > 4.0) {
      ...
      // Smooth turning instead of instant snap
      if ((Boolean)this.entityData.get(IS_IN_COMBAT) && !this.isDeadOrDying()) {
         float targetYaw = (Float)this.entityData.get(COMBAT_YAW);
         float currentYaw = this.yBodyRot;
         float diff = Mth.wrapDegrees(targetYaw - currentYaw);
         if (Math.abs(diff) > MAX_TURN_SPEED) {
            diff = Math.signum(diff) * MAX_TURN_SPEED;
         }
   ...
   public boolean hurt(DamageSource source, float amount) {
      ...
      if (attacker instanceof AbstractUnit otherUnit) {
         if (otherUnit.getFaction() == this.getFaction()) {
            return false;
         }
      }
      ...
      if (damageOccurred && !this.level().isClientSide()) {
         ...
         // If hit by player, seek cover immediately
         if (attacker instanceof net.minecraft.world.entity.player.Player) {
            this.setSoldierState(SoldierState.SEEK_COVER);
         }
      }
      if (damageOccurred && this.level().isClientSide()) {
   ...
   public void setTarget(@Nullable LivingEntity newTarget) {
      if (newTarget instanceof AbstractUnit otherUnit) {
         if (otherUnit.getFaction() == this.getFaction()) {
            return;
         }
      }
   ...
   public BlockPos getHomePoint() {
      BlockPos pos = (BlockPos)this.entityData.get(HOME_POS);
      return pos.equals(BlockPos.ZERO) ? null : pos;
   }

   public void setFaction(FactionType faction) {
      this.entityData.set(FACTION_TYPE, faction.name());
   }

   public FactionType getFaction() {
      try {
         return FactionType.valueOf((String)this.entityData.get(FACTION_TYPE));
      } catch (IllegalArgumentException e) {
         return FactionType.TOWER;
      }
   }

   public boolean isTargetLostTimeoutExpired() {
      long lastSeen = (Long)this.entityData.get(LAST_SEEN_TARGET_TIME);
      long currentTime = this.level().getGameTime();
      return lastSeen > 0 && (currentTime - lastSeen) > TARGET_LOST_RETURN_DELAY;
   }

   public boolean isTooFarFromHome() {
      BlockPos home = getHomePoint();
      if (home == null) return false;
      return this.distanceToSqr(Vec3.atCenterOf(home)) > HOME_MAX_DISTANCE_SQR;
   }
}