   ...
   public void setupRoleGoals() {
      ...
      this.targetSelector.addGoal(2, new VerticalAwareTargetGoal<>(this, Monster.class, true, vRange, target -> {
         if (target.getClass() == this.getClass()) {
            ...
            return target instanceof AbstractUnit ? true : target instanceof Monster;
         }
      }));
      switch (this.getRole()) {
         case HOSTILE:
   ...
   public InteractionResult mobInteract(Player player, InteractionHand hand) {
      if (!this.level().isClientSide && hand == InteractionHand.MAIN_HAND) {
         if (this.getOwnerUUID() == null) {
            ...
            player.displayClientMessage(Component.literal("[SEM] Unit Recruited! Awaiting orders."), false);
            this.playSound(SoundEvents.NOTE_BLOCK_PLING.get(), 1.0F, 1.0F);
            return InteractionResult.SUCCESS;
         }

         if (!this.isOwnedBy(player)) {
            player.displayClientMessage(Component.literal("[SEM] Unit Status: " + this.getOrder().name()), false);
            return InteractionResult.SUCCESS;
         }
