   ...
   public InteractionResult mobInteract(Player player, InteractionHand hand) {
      if (!this.level().isClientSide && hand == InteractionHand.MAIN_HAND) {
         if (this.getOwnerUUID() == null) {
            ...
            player.displayClientMessage(Component.literal("§a[SEM] Unit Recruited! Awaiting orders."), false);
            this.playSound(SoundEvents.NOTE_BLOCK_PLING.get(), 1.0F, 1.0F);
            return InteractionResult.SUCCESS;
         }

         if (!this.isOwnedBy(player)) {
            player.displayClientMessage(Component.literal("§e[SEM] Unit Status: " + this.getOrder().name()), false);
            return InteractionResult.SUCCESS;
         }
