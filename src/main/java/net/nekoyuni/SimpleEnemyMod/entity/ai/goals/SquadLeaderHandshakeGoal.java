...
public class SquadLeaderHandshakeGoal extends Goal {
   ...
   public boolean canUse() {
      return this.isSquadLeader(this.leader) && !this.leader.isUsingItem() && this.leader.getTarget() == null;
   }
   ...
   private List<PathfinderMob> findNearbyUnassignedUnits() {
      ...
      return this.leader
         ...
         .getEntitiesOfClass(
            PathfinderMob.class,
            searchBox,
            entity -> this.isSquadUnit(entity) && this.isSameFaction(this.leader, entity) && !SquadData.hasValidSquadData(entity)
         );
   }

   private boolean canAcceptUnit(PathfinderMob unit) {
      int MAX_SQUAD_SIZE = 8;
   ...
   private boolean isSquadLeader(PathfinderMob entity) {
      return !(entity instanceof IRoleHolder roleHolder)
         ? false
         : roleHolder.getRole() == UnitRole.SQUAD_LEADER || roleHolder.getRole() == UnitRole.FRIENDLY_SQUAD_LEADER;
   }

   private boolean isSquadUnit(PathfinderMob entity) {
      return !(entity instanceof IRoleHolder roleHolder)
         ? false
         : roleHolder.getRole() == UnitRole.SQUAD_UNIT || roleHolder.getRole() == UnitRole.FRIENDLY_SQUAD_UNIT;
   }
